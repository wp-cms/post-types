...
// Enqueue admin CSS
add_action( 'admin_enqueue_scripts', 'Core\PostTypes\enqueue_styles' );

// admin setup
add_action( 'admin_init', 'Core\PostTypes\post_types_settings_flush_rewrite' );
add_action( 'add_meta_boxes', 'Core\PostTypes\post_types_metabox');
add_action( 'save_post', 'Core\PostTypes\post_types_save_metabox');
add_filter( 'post_updated_messages', 'Core\PostTypes\post_types_update_messages');

// table columns
add_filter( 'manage_post_types_posts_columns', 'Core\PostTypes\post_types_posts_columns');
add_action( 'manage_post_types_posts_custom_column', 'Core\PostTypes\post_types_posts_custom_columns', 10, 2);
...
function post_types_metabox() {
    ...
    add_meta_box(
        ...
        'Core\PostTypes\post_types_metabox_content',
        ['post_types'],
        'advanced',
        'high'
    );
    ...
    add_meta_box(
        ...
        'Core\PostTypes\post_types_metabox_side',
        ['post_types'],
        'side',
        'low'
    );
...
function post_types_metabox_side( $post ) {
    ?>
    <p><?php _e( 'For more info about each field please check', 'post-types');?> <a href="https://developer.wordpress.org/reference/functions/register_post_type/" target="_blank"><?php _e( 'the official documentation.', 'post-types');?></a></p>
    <?php
}
...
function post_types_metabox_content( $post ) {
    ...
    $post_type_supports_trackbacks      = ( isset( $meta['post_type_supports'] ) && in_array( 'trackbacks', $post_type_supports ) ? 'trackbacks' : '' );
    $post_type_supports_custom_fields   = ( isset( $meta['post_type_supports'] ) && in_array( 'custom-fields', $post_type_supports ) ? 'custom-fields' : '' );
    $post_type_supports_comments        = ( isset( $meta['post_type_supports'] ) && in_array( 'comments', $post_type_supports ) ? 'comments' : '' );
...
function post_types_posts_custom_columns( $column_name, $id ) {
    ...
    if ( $column_name === 'slug' ) {
        echo get_post_meta( get_the_ID(), 'post_type_name', true );
    }
    if ( $column_name === 'description' ) {
        echo get_post_meta( get_the_ID(), 'post_type_description', true );
    }
}
