...
add_action( 'init', 'plugin\post_types\register_post_types' );
	...
	if ( $post_types_definitions ) {
		foreach ( $post_types_definitions as $post_type ) {
			...
			if ( $post_type_rewrite ) {
				$post_type_rewrite_options['slug'] = $post_type_custom_rewrite_slug;
			}

			$post_type_rewrite_options['with_front'] = $post_type_withfront;

			if ( $post_type_feeds ) {
				$post_type_rewrite_options['feeds'] = true;
			}
			if ( $post_type_pages ) {
				$post_type_rewrite_options['pages'] = true;
			}
			...
			if ( is_array( $post_types ) ) {
				foreach ( $post_types as $post_type ) {
					...
					$labels = array(
						'name'               => $post_type['post_type_label'],
						'singular_name'      => $post_type['post_type_singular_name'],
						'add_new'            => __( 'Add New', 'post-types' ),
						                        /* translators: 1: Singular Name */
						'add_new_item'       => sprintf( __( 'Add New %s', 'post-types' ), $post_type['post_type_singular_name'] ),
						                        /* translators: 1: Singular Name */
						'edit_item'          => sprintf( __( 'Edit %s', 'post-types' ), $post_type['post_type_singular_name'] ),
						                        /* translators: 1: Singular Name */
						'new_item'           => sprintf( __( 'New %s', 'post-types' ), $post_type['post_type_singular_name'] ),
						                        /* translators: 1: Singular Name */
						'view_item'          => sprintf( __( 'View %s', 'post-types' ), $post_type['post_type_singular_name'] ),
						                        /* translators: 1: Post Type Label */
						'search_items'       => sprintf( __( 'Search %s', 'post-types' ), $post_type['post_type_label'] ),
						                        /* translators: 1: Post Type Label */
						'not_found'          => sprintf( __( 'No %s found', 'post-types' ), $post_type['post_type_label'] ),
						                        /* translators: 1: Post Type Label */
						'not_found_in_trash' => sprintf( __( 'No %s found in Trash', 'post-types' ), $post_type['post_type_label'] ),
					);
